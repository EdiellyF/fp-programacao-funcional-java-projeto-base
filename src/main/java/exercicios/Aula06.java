...
public class Aula06 extends Aula {
     ...
     */
    private final Predicate<Estudante> mulheresAprovadas = ((Predicate<Estudante>) e -> e.getSexo() == 'F')
    ...
    public List<Estudante> getEstudantesMulheresAprovadasOrdenadasPorCursoDecrescenteAndNotaCrescente() {
        return this.estudantes.stream()
                .filter(mulheresAprovadas)
                .sorted(
                        Comparator.comparing((Estudante e) -> e.getCurso().getNome(), Comparator.reverseOrder())
                                .thenComparingDouble(Estudante::getNota)
                )
                .toList();
    }
